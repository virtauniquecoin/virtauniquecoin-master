...
{
    QString ret = QString("virtauniquecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VUC unit input in lnReqAmount, we generate the URI with VUC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(virtauniquecoinUnits::format(virtauniquecoinUnits::VUC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
