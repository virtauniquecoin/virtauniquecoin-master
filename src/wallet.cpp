...
// virtauniquecoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-virtauniquecoin-address
...
// VirtaUniqueCoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CvirtauniquecoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CvirtauniquecoinAddress(address).ToString());
}
...
// virtauniquecoin: check 'spent' consistency between wallet and txindex
// virtauniquecoin: fix wallet spent state according to txindex
...
// virtauniquecoin: disable transaction (only for coinstake)
